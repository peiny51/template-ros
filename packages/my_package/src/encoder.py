    ...
    def cb_encoder_data(self, msg, wheel):
        ...
        if wheel == 'left':
            if self.initial_left_tick:
            	self.previous_left_wheel_ticks = msg.data
            	self.initial_left_tick = False
            	
            left_dist = (msg.data - self.prevoius_left_wheel_ticks) * self._radius * 2 * math.pi / N_total
            ...
            if self.initial_right_tick:
            	self.previous_right_wheel_ticks = msg.data
            	self.initial_right_tick = False

            right_dist = (msg.data - self.prevoius_right_wheel_ticks) * self._radius * 2 * math.pi / N_total
            self.right_dist += self.right_dist
            self.previous_right_wheel_ticks = msg.data
